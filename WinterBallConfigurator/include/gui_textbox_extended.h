...
RAYGUIDEF bool GuiTextBoxEx(Rectangle bounds, char *text, int textSize, bool editMode);/*
 __  __  ____  _____ _____ ______ _____ ______ _____
 |  \/  |/ __ \|  __ \_   _|  ____|_   _|  ____|  __ \
 | \  / | |  | | |  | || | | |__    | | | |__  | |  | |
 | |\/| | |  | | |  | || | |  __|   | | |  __| | |  | |
 | |  | | |__| | |__| || |_| |     _| |_| |____| |__| |
 |_|  |_|\____/|_____/_____|_|    |_____|______|_____/
 
 Add the below two functions to make there be no errors when compiling. The functions exist in raylib just not here
*/
...
/*
 ...
 Added some `(float)` to fix invalid cast warning.
*/
...
*/
    ...
    Replace `TEXTEDIT_CURSOR_BLINK_FRAMES` with 30 to make the cursor blink every second, TEXTEDIT_CURSOR_BLINK_FRAMES was undefined

    Replace a call with `DrawTextRecEx`
    */
